...
//hämtar recensionstext och knapp och området där recensioner visas
const reviewBox = document.querySelector(".recensioner-box");
const submitBtn = document.querySelector(".submit-btn");
const reviewList = document.getElementById("reviewl");

//när man klickar på knappen
submitBtn.addEventListener("click", () => {
  //gör texten säker
  const reviewText = sanitize(reviewBox.value);

  //kontroll så att recensionen inte är tom
  if (reviewText === "") {
    alert("Skriv en recension först!");
    return;
  }

  //skapa nytt recensionselement
  const review = document.createElement("p");
  review.textContent = reviewText;

  //lägger till recensionen i själva listan
  reviewList.appendChild(review);

  //tömmer textboxen efter manskickat recension
  reviewBox.value = "";
});
